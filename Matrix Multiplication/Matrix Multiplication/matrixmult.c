...
/* Initialize MPI COMM WORLD, SET RANK AND SIZE */
    MPI_Init(NULL, NULL);
    comm = MPI_COMM_WORLD;
  ...
  /* Allocate the memory for local matricies to be operated on */
    Allocate_memory(&local_A, &local_B, &local_C, my_rank, n, local_n, comm);
    a_part = (n*n)/comm_sz;
    // printf("a_part = %i\n", a_part);
/* Seed random number generator */
    srandom(time(NULL));
/* Allocate B and fill with zeroes */
    B = malloc(n*n*sizeof(int));
    memset(B, 0, n*n*sizeof(int));
    MPI_Barrier(MPI_COMM_WORLD);

  if (my_rank == 0) {
/* Allocate A and C and Fill with zeroes */
  ...
  /* Start the timer, then broadcast B and scatter A */
  MPI_Barrier(MPI_COMM_WORLD);
  start = MPI_Wtime();
  ...
  //  printf( "Form = %i\n", *form);
   if ( *form == 1) {
      ...
      //  printf("KIJ Form\n");
       ...
       printf("Invalid <form>\n");
/* Gather all of the local_C result vectors onto the C held by p0 */
   MPI_Gather(local_C, local_n*n, MPI_INT, C, local_n*n, MPI_INT, 0, comm);
   finish = MPI_Wtime();
   loc_elapsed = finish-start;
/* Reduce the cocal times onto elapsed time */
